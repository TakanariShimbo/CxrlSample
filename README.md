...
- リファクタ: glass 側 client app の起動/終了 (`appStart` / `appStop`) を CustomCmd 画面のライフサイクルに紐付け。元サンプルでは親画面 (CustomAppType) に `Open app scene` / `Stop app scene` のボタンを置き、scene を開いた後に Audio/Photo/CustomCmd の各サブ画面に入る前提だったが、調査の結果 Audio (`startAudioStream`) と Photo (`takePhoto`) は SDK の AIDL サービス層で完結し client app 不要、必要なのは custom cmd の送受信 (`sendCustomCmd` / `setCXRCustomCmdCbk` の peer 側受信機) だけと判明したため、`CustomCmdViewModel` の `init()` で `appStart` / `onCleared()` で `appStop` を呼ぶ構造に変更し、親画面のシーン関連 UI と関連 string (`custom_app_open_scene` / `custom_app_stop_scene` / `custom_app_scene_status` 等) を削除。`appStart` / `appStop` の呼び出し自体は本家 SDK の API そのままで、引数も同じ (`"${APP_PACKAGE_NAME}${MAIN_PAGE}"`)。SDK API カバレッジは維持。config change 時の二重 `appStart` を避けるため `CustomCmdViewModel` 内に `sceneStarted` フラグを追加
